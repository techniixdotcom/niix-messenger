...
## ~~Blocker 1: there is no LICENSE file~~ (resolved)

A `LICENSE` file with the full GPL-3.0 text is now at the repository root, and
`metadata/app.niix.yml` declares `GPL-3.0-only` to match. Reviewers check that the declared
licence matches what is actually in the repo, and it now does.

Kept below for the reasoning, since the choice is effectively irreversible once others have the
code and is worth understanding rather than inheriting.
