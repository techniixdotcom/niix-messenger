...
/**
 * Raised when an identity key change is attempted for a contact whose key the user pinned by
 * scanning their code. Deliberately an exception rather than a silently-ignored write: the
 * session establishment that triggered it must fail, not quietly continue against a key that
 * disagrees with what the user verified.
 */
internal class UntrustedIdentityChangeException(addressName: String) :
    IllegalStateException("Identity key change refused for a pinned contact: $addressName")

internal class DatabaseSignalProtocolStore(
    private val secureDatabase: SecureDatabase,
    private val identityManager: IdentityManager,
    /** Returns the key the user pinned for this address, or null if none was ever pinned.
     * Supplied by the messaging layer, which owns the pin. */
    private val pinnedIdentityProvider: (String) -> ByteArray? = { null },
) : SignalProtocolStore {
    ...
    override fun saveIdentity(
        ...
        // Refuse to overwrite an identity the user explicitly pinned by scanning a contact's
        // code.
        //
        // Previously the pin was checked by the application *after* decryption had already
        // happened, which made identity continuity depend on an ordering relationship between
        // a libsignal callback and a separate later check. Enforcing it here makes the identity
        // store itself authoritative: a pinned key cannot be replaced at all, so no amount of
        // session churn or ordering subtlety can get a substituted identity accepted.
        //
        // The pin is stored by the messaging layer, so it is read through a callback rather than
        // this module reaching upward for it.
        val pinned = pinnedIdentityProvider(address.name)
        if (pinned != null && !pinned.contentEquals(identityKey.serialize())) {
            throw UntrustedIdentityChangeException(address.name)
        }
    ...
    /**
     * Deletes *every* sender-key record held for [sender], regardless of distribution id.
     *
     * [deleteSenderKey] can only remove distributions this device recorded in its own tracking
     * table, so revocation was only ever as complete as that bookkeeping. Anything that arrived
     * before the tracking existed, or whose record failed to write, would survive removal and
     * keep decrypting that member's traffic -- silently, since nothing reports a key that wasn't
     * revoked. Removing by sender alone makes revocation depend on nothing but the sender's
     * identity, which is the property that actually matters when someone is ejected from a
     * group.
     *
     * Safe despite ignoring the group: a sender key is per-(sender, distribution), and a fresh
     * distribution is issued on the next epoch change in any group this sender is still part of.
     * The cost of over-deleting is one redistribution; the cost of under-deleting is a removed
     * member who can still read the conversation.
     */
    fun deleteAllSenderKeysFrom(sender: SignalProtocolAddress) {
        db.delete(
            Schema.GroupSenderKeys.TABLE,
            "${Schema.GroupSenderKeys.COL_SENDER_NAME} = ?",
            arrayOf(sender.name),
        )
    }
