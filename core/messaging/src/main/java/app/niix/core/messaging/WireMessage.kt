...
sealed class WireMessage {
     ...
     *
     * Deliberately its own message rather than a GroupInvite omitting the sender. A membership
        ...
        DataInputStream(ByteArrayInputStream(bytes)).use { s ->
            ...
            return when (val type = s.readByte().toInt()) {
                TYPE_TEXT -> WireMessage.Text(
                    ...
                    replyToId = runCatching { readOptionalString(s) }.getOrNull(),
                )
                ...
                TYPE_GROUP_INVITE -> WireMessage.GroupInvite(
                    ...
                    prevStateHash = runCatching { readOptionalString(s) }.getOrNull(),
                )
                ...
                TYPE_PROFILE -> WireMessage.ProfileUpdate(
                    ...
                    // previous version.
                    displayName = runCatching { readOptionalString(s) }.getOrNull(),
                )
