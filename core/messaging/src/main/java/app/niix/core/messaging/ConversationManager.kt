...
class ConversationManager(
     ...
     * wipe -- this module deliberately cannot erase the database it is reading from. */
     ...
     */
    suspend fun sendRemoteWipe(peerOnion: String, token: String) = withContext(Dispatchers.IO) {
        runCatching {
            ensureSession(peerOnion)
            sendWireDirectOrRelay(peerOnion, WireMessage.RemoteWipe(token))
        }
        Unit
    }
     ...
     *
     * Deliberately nullable rather than falling back to an empty array. An empty key still
     ...
     * for telling people apart. Names are deliberately not unique or claimed: two people may
    ...
    private fun dispatch(fromOnion: String, wire: WireMessage, viaRelay: Boolean = false) {
        when (wire) {
            ...
            is WireMessage.RemoteWipe -> {
                ...
                if (storage.remoteWipe.shouldWipe(fromOnion, wire.token)) {
