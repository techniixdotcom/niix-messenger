...
class ConversationManager(
 ...
 // checked against and the chain silently starts one transition late.
    ...
    /**
     * Marks a single message read, starting its disappearing timer and sending the receipt.
     *
     * Needed alongside markConversationRead because with blurring on, opening a conversation no
     * longer means the messages in it were read, they are still hidden. Starting every timer
     * on open would expire messages the user never actually saw.
     */
    /**
 ...
 // returns early, and it still needs to stay unblurred.
    ...
    /**
     * Changes the name contacts see, and tells them about it.
     *
     * Trimmed and length-capped for the same reason names received from others are: this one is
     * sent to every contact and rendered in their conversation list, so an overlong or
     * whitespace-padded name would disrupt their UI rather than this device's.
     *
     * Contacts are told immediately rather than on next contact. A rename nobody learns about is
     * indistinguishable from one that did not happen, and the most likely reason to change a
     * displayed name is that the old one had become a problem.
     */
    ...
    suspend fun drainLockedInbox() = withContext(Dispatchers.IO) {
        ...
        for (entry in queued) {
            runCatching { handleIncoming(entry.senderOnion, entry.ciphertext, entry.viaRelay) }
        }
        notifyChanged()
    }
 ...
 // A name the user has chosen locally wins; only fill in one that was never set.
    ...
    /**
     * Decrypts an attachment, falling back to the unbound form for files written before the id
     * was included as associated data.
     *
     * Without the fallback, enabling this would make every existing attachment permanently
     * unreadable, the bytes are fine, but the authentication tag no longer matches. The
     * fallback is only reached when the bound attempt fails, so new files gain the protection
     * immediately while old ones keep working.
     */
    /**
 ...
 // A partial write may already exist; it is not the file the caller asked for.
    ...
    /**
     * Retires an attachment whose ciphertext can no longer be decrypted.
     *
     * These are files encrypted before the attachment id was bound in as associated data. The
     * bytes are intact but nothing can ever read them again, so keeping them means storage that
     * only grows and a UI that looks broken rather than explaining itself.
     *
     * Deleting the blob is safe precisely because the failure is permanent: it is not a transient
     * error that might succeed on retry, and it is not a wrong-key case that could be recovered
     * later, the key is right, the binding is absent, and no future version brings it back.
     *
     * The record is kept and marked FAILED rather than removed, so the conversation still shows
     * that something was sent. Deleting the row would rewrite history, which is worse than
     * admitting a file is gone.
     */
 ...
 // Silence is the only safe response in both directions.
                if (storage.remoteWipe.shouldWipe(fromOnion)) {
 ...
 // confusing at best and at worst invites wiping a device that never asked.
                if (storage.contacts.isKnown(fromOnion)) {
 ...
 // who inherits authority.
    ...
    /**
     * The hash of the group state this device currently believes is in force.
     *
     * Held in settings rather than a new column so this can ship without a schema migration.
     * It is written whenever a state is accepted, and read when sending a transition, so an
     * outgoing change always names the state it was actually built on rather than a state
     * reconstructed after the local mutation has already happened.
     */
