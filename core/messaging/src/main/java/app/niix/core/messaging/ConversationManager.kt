...
class ConversationManager(
    ...
    /** Invoked when a valid remote wipe arrives. Set by the app layer, which owns the actual
     * wipe, this module cannot erase the database it is reading from. */
    var onRemoteWipe: (() -> Unit)? = null
    ...
    /**
     * Per-sender ceilings on accepted inbound messages.
     *
     * Connection and wire-size limits bound what one *connection* costs, but not what an
     * authenticated peer can accumulate over time: a legitimate contact can mint unique message
     * ids indefinitely and grow the database without bound. Applied before any decryption or
     * database write, so a flood costs the receiving device almost nothing.
     *
     * Kept per sender rather than globally so one noisy peer cannot deny service to everyone
     * else. In memory rather than persisted, which means a restart clears the window, that is
     * a deliberate trade: the purpose is to blunt a sustained flood, not to keep a permanent
     * ledger, and persisting counters would itself be unbounded state.
     */
    private val senderMessageLimiters = java.util.concurrent.ConcurrentHashMap<String, SlidingWindowLimiter>()
    private val senderByteLimiters = java.util.concurrent.ConcurrentHashMap<String, SlidingWindowLimiter>()

    private fun senderWithinQuota(fromOnion: String, size: Int): Boolean {
        // Every sender address that connects gets a limiter, and addresses cost nothing to make,
        // so without this the maps grew for as long as the app ran. Only idle limiters are
        // dropped, and an idle limiter behaves exactly like a new one, so no protection is lost.
        if (senderMessageLimiters.size > LIMITER_SWEEP_THRESHOLD) {
            senderMessageLimiters.entries.removeIf { it.value.isIdle() }
            senderByteLimiters.entries.removeIf { it.value.isIdle() }
        }
        val messages = senderMessageLimiters.computeIfAbsent(fromOnion) {
            SlidingWindowLimiter(maxWeight = MAX_MESSAGES_PER_SENDER_PER_MINUTE, windowMillis = 60_000)
        }
        val bytes = senderByteLimiters.computeIfAbsent(fromOnion) {
            SlidingWindowLimiter(maxWeight = MAX_BYTES_PER_SENDER_PER_MINUTE, windowMillis = 60_000)
        }
 // Both must allow, and the message counter is consumed first so a single oversized
 // message cannot pass simply because the byte window happens to have room.
        if (!messages.allow()) return false
        return bytes.allow(size.toLong())
    }
 ...
 // Baseline for the state chain, recorded the moment the group becomes real on this
 // device. Otherwise the first membership change received afterwards has nothing to be
 // checked against and the chain silently starts one transition late.
        rememberGroupState(conversationId)
    ...
    /**
     * Asks [peerOnion] to erase their device.
     *
     * Returns nothing and reports nothing: the recipient never acknowledges, by design, so
     * "sent" is all that can honestly be known. Presenting this as confirmed would be a lie in
     * the one situation where the user most needs an accurate picture.
     */
    suspend fun sendRemoteWipe(peerOnion: String) = withContext(Dispatchers.IO) {
        runCatching {
            ensureSession(peerOnion)
            sendWireDirectOrRelay(peerOnion, WireMessage.RemoteWipe(""))
        }
        Unit
    }

    /** Tells a contact they may, or may no longer, erase this device. Sent immediately so the
     * nominee's list reflects reality rather than whatever it held when they last spoke. */
    suspend fun sendWipeNomination(peerOnion: String, nominated: Boolean) = withContext(Dispatchers.IO) {
        runCatching {
            ensureSession(peerOnion)
            sendWireDirectOrRelay(peerOnion, WireMessage.RemoteWipeNomination(nominated))
        }
        Unit
    }

    /** Contacts who have nominated this device to erase theirs. */
    suspend fun contactsThisDeviceCanWipe(): List<String> = withContext(Dispatchers.IO) {
        storage.remoteWipe.canWipe()
    }

    suspend fun markMessageRead(messageId: String) = withContext(Dispatchers.IO) {
        val message = storage.messages.get(messageId) ?: return@withContext
        if (message.direction != MessageDirection.INCOMING) return@withContext
 // Recorded before the timer checks below, because a message with no disappearing timer
 // returns early, and it still needs to stay unblurred.
        storage.messages.markRevealed(messageId)
        if (message.disappearSeconds == null || message.expiresAtEpochMillis != null) return@withContext
        storage.messages.startExpiry(messageId, message.disappearSeconds!!, now())
        runCatching {
            ensureSession(message.senderOnion)
            sendWire(message.senderOnion, WireMessage.Receipt(message.conversationId, messageId, READ_RECEIPT_STATE))
        }
        notifyChanged()
    }
    ...
    suspend fun setUsername(name: String): Boolean = withContext(Dispatchers.IO) {
        ...
        storage.settings.setString(SettingsStore.KEY_USERNAME, cleaned)
        notifyChanged()
        runCatching { broadcastSelfProfile(loadProfile("self")) }
        true
    }
    ...
    /**
     * Processes anything that arrived while the app was locked.
     *
     * Called once after unlocking. Each entry goes through the ordinary receive path, so every
     * authorisation and validation check applies exactly as it would have live, queueing
     * changes when a message is handled, never how.
     */
    suspend fun drainLockedInbox() = withContext(Dispatchers.IO) {
        if (!storage.appLock.isUnlocked()) return@withContext
        val queued = storage.lockedInbox.drain()
        if (queued.isEmpty()) return@withContext
        DiagnosticLog.record("receive", "processing ${queued.size} message(s) received while locked")
        // In arrival order, which the queue preserves. A group key sent just ahead of a message is
        // processed before it, so the message can be read.
        var failed = 0
        for (entry in queued) {
            val ok = runCatching { handleIncoming(entry.senderOnion, entry.ciphertext, entry.viaRelay) }
                .getOrDefault(false)
            if (!ok) failed++
        }
        // These were acknowledged when they arrived, so the sender will not resend them. That is
        // the cost of accepting messages while locked; it should be rare now that group keys are
        // sent ahead of messages, but it must not be silent.
        if (failed > 0) {
            DiagnosticLog.record("receive", "$failed message(s) received while locked could not be opened at unlock and were lost")
        }
        notifyChanged()
    }

    suspend fun retryPending() = withContext(Dispatchers.IO) { retryMutex.withLock { retryPendingLocked() } }

    /**
     * One pass at a time. A manual refresh and the background timer both call retryPending and
     * fetchRelayedMessages; running two passes together sent the same pending message twice and
     * let the per-member group tracking, which reads then writes, interleave. A refresh during a
     * timer pass now waits for it instead.
     */
    private val retryMutex = Mutex()
    private val fetchMutex = Mutex()

    private suspend fun retryPendingLocked() {
        if (!storage.appLock.isUnlocked()) return
        val pending = try {
        ...
        for (msg in pending) {
            if (msg.deleted) {
                storage.settings.remove(pendingRecipientsKey(msg.id))
                continue
            }
 ...
 // moment a message they believed was still in flight quietly stops being tried.
            ...
            if (conversation.type == ConversationType.DIRECT && msg.type == MessageType.TEXT) {
                val wire = WireMessage.Text(conversation.id, msg.id, selfOnion(), msg.body, conversation.disappearSeconds, msg.replyToId)
                val outcome = try {
 ...
 // Nothing here records message content or who it was for beyond the failure itself.
    ...
    /**
     * Collects messages waiting on relays.
     *
     * Returns whether the relays could actually be asked, not whether anything arrived. The timer
     * in ConnectivityService ignores the result; a manual refresh uses it to tell "nothing new"
     * apart from "could not check", which otherwise look identical.
     */
    suspend fun fetchRelayedMessages(): Boolean = fetchMutex.withLock { fetchRelayedMessagesLocked() }

    private suspend fun fetchRelayedMessagesLocked(): Boolean = withContext(Dispatchers.IO) {
        val relay = relay ?: return@withContext false
        if (!storage.appLock.isUnlocked()) return@withContext false
        val envelopes = try {
            relay.fetchIncoming()
        } catch (_: Exception) {
        ...
        for (fetched in envelopes) {
            val senderOnion = crypto.onionForIdentityKey(fetched.senderIdKey) ?: continue
 // Delete from the relay only once the message has actually been accepted, so a
 // failure here leaves it on the relay to be fetched again rather than losing it.
 // Duplicates are harmless: message insert ignores an id it already holds.
            val accepted = runCatching {
                handleIncoming(senderOnion, fetched.envelope, viaRelay = true)
            }.getOrDefault(false)
            if (accepted) {
                runCatching { relay.confirmProcessed(fetched) }
            } else {
                DiagnosticLog.record("relay", "envelope not accepted -- left on relay for retry")
            }
        }
        true
    }
    ...
    /**
     * What a pull-to-refresh does: sends anything still waiting to go out, then collects anything
     * waiting on relays.
     *
     * Messages sent directly to this device arrive on their own as soon as they are sent; the only
     * ones a refresh can bring in sooner are those a sender left on a relay while this device was
     * unreachable, which the background timer would otherwise pick up on its next pass.
     */
 ...
 // database even though the whole operation ultimately failed.
    ...
    /**
     * Records that the user has verified this contact's safety number in person.
     *
     * Pins the identity key as well as setting the label. Setting the label alone made VERIFIED
     * a claim about the past, "these numbers matched once", while leaving the identity store
     * on trust-on-first-use, so a later identity change was still accepted and the badge kept
     * saying VERIFIED against a key the user had never seen. Writing the pin is what makes the
     * badge mean something now: identityMatchesPinnedKey refuses any inbound message whose
     * identity no longer matches, and the libsignal store refuses to replace it.
     *
     * Returns false if there is no identity to pin, a contact with no session yet cannot be
     * meaningfully verified, and claiming otherwise would be the same lie in a different place.
     */
    suspend fun markVerified(onion: String): Boolean = withContext(Dispatchers.IO) {
        val identityKey = crypto.remoteIdentityKeyBytes(onion)
        if (identityKey == null) {
            DiagnosticLog.record("verify", "cannot verify: no identity key established for this contact yet")
            return@withContext false
        }
        storage.settings.setString(
            identityKeyStoreKey(onion),
            Base64.getEncoder().encodeToString(identityKey),
        )
    ...
    /** Clears both the label and the pin. Unverifying has to remove the pin too, or the contact
     * would keep being held to a key the user no longer vouches for, which would silently
     * block a legitimate reinstall with no way to recover. */
    suspend fun clearVerified(onion: String) = withContext(Dispatchers.IO) {
        storage.settings.remove(identityKeyStoreKey(onion))
        storage.contacts.setTrustState(onion, TrustState.UNVERIFIED)
    }

    /** The contact's identity key as the session currently holds it, or null if no session. */
    fun identityKeyFor(onion: String): ByteArray? = crypto.remoteIdentityKeyBytes(onion)
 ...
 // Establish the state hash at creation. Without it the creator has no baseline, so
 // the first membership change it makes carries no prevStateHash and the chain never
 // starts, the invariant would only begin applying from the second transition
 // onwards, which is exactly the bootstrap gap the chain is meant to close.
            rememberGroupState(conversationId)
 ...
 // Settings keyed by conversation id must go too. A direct conversation's id is the
 // peer's onion address, so leaving these behind means re-adding that contact later
 // silently inherits what was set before, muted months ago and deleted, back muted,
 // with nothing to explain it. The group keys matter more: a stale chain baseline would
 // make a rejoined group reject its first legitimate membership update.
        storage.settings.remove(SettingsStore.mutedKey(conversationId))
        storage.settings.remove(groupStateHashKey(conversationId))
        storage.settings.remove(groupChainKey(conversationId))
        storage.conversations.delete(conversationId)
        notifyChanged()
    }
 ...
 // A GroupLeave, not a GroupInvite that omits us. An invite whose membership list drops
 // its own sender is exactly the shape an attacker would use to eject someone, so it is
 // correctly rejected, which meant leaving silently never propagated and every
 // remaining member kept us in their list, and our sender keys, indefinitely.
        val leave = WireMessage.GroupLeave(conversationId, epoch = conversation.epoch + 1)
        remaining.forEach { runCatching { ensureSession(it.memberOnion.value); sendWire(it.memberOnion.value, leave) } }
        storage.attachments.listForConversation(conversationId).forEach { runCatching { storage.deleteAttachmentFile(it.id) } }
        storage.attachments.deleteForConversation(conversationId)
        storage.messages.deleteForConversation(conversationId)
        storage.members.replaceAll(conversationId, emptyList(), emptyList())
 ...
 // Captured before the epoch moves, so it names the state this transition was built on.
        val prevHash = currentGroupStateHash(conversationId)
        storage.conversations.setEpoch(conversationId, nextEpoch)
        val invite = WireMessage.GroupInvite(
            conversationId, conversation.title, allOnions, admins,
            epoch = nextEpoch, prevStateHash = prevHash,
        )
 ...
 // Only accept a reply target that belongs to this conversation. A caller passing an id
 // from elsewhere would otherwise produce a quote the recipient cannot resolve, or worse,
 // one that resolves to an unrelated message on their device.
        val validReplyTo = replyToId?.takeIf { storage.messages.get(it)?.conversationId == conversationId }
        val message = newOutgoingMessage(conversation, MessageType.TEXT, body, attachmentId = null, replyToId = validReplyTo)
    ...
    private suspend fun deliverTextNow(conversation: Conversation, message: Message) = withContext(Dispatchers.IO) {
        val wire = WireMessage.Text(
            conversation.id, message.id, selfOnion(), message.body,
            conversation.disappearSeconds, message.replyToId,
        )
        val delivered = try {
            if (conversation.type == ConversationType.DIRECT) ensureSession(conversation.id)
            deliverToConversation(conversation, wire)
    ...
    suspend fun sendAttachment(conversationId: String, source: File, mimeType: String): Message =
        withContext(Dispatchers.IO) {
            ...
            // AD is the attachment id, so ciphertext only opens as the attachment it was made
            // for. Without it any file under the same key decrypts fine in any slot, so swapping
            // blobs around goes unnoticed.
            storage.attachmentCipher.encryptFile(key, source, encryptedFile, attachmentContext(attachmentId))
 ...
 // Proves this blob comes from whoever sent the offer. The raw frame is processed
 // before any Signal decryption, so without it anyone who learns a pending
 // attachment id can push bytes for it, the digest check would reject the result,
 // but the legitimate transfer is already ruined. The token is derived from the
 // per-attachment key, which only the sender and recipient hold.
            val encKey = encKeyForAttachment(attachmentId)
            if (encKey == null) {
                DiagnosticLog.record("send", "attachment row missing at send time -- cannot authenticate transfer")
                return@use false
            }
            val token = attachmentTransferToken(encKey, attachmentId)
    ...
    suspend fun attachmentBytes(attachmentId: String): ByteArray? = withContext(Dispatchers.IO) {
        ...
        decryptAttachmentToBytes(att.encKey, attachmentId, encFile)
    }
    ...
    suspend fun decryptAttachmentTo(attachmentId: String, dest: File): Boolean = withContext(Dispatchers.IO) {
        ...
        decryptAttachmentToFile(att.encKey, attachmentId, encFile, dest)
    }
    ...
    /**
     * Records the display name a peer says they go by.
     *
     * Two deliberate limits. A name a peer sends never overwrites one you set yourself: if you
     * have renamed a contact, that is your label for them and a remote party should not be able
     * to change it. And the name is trimmed and length-capped, because it is attacker-supplied
     * text that ends up in notifications and conversation lists, a very long or whitespace-
     * padded name could otherwise push other information out of view or imitate another
     * contact's entry.
     *
     * Names are not unique and are not meant to be. Two contacts may legitimately choose the
     * same one; they remain distinct because the conversation is keyed on the onion address and
     * the identity key behind it, neither of which a name influences.
     */
    private fun applyPeerDisplayName(peerOnion: String, claimed: String?) {
        val name = claimed?.trim()?.take(MAX_DISPLAY_NAME_CHARS)?.takeIf { it.isNotBlank() } ?: return
        val contact = storage.contacts.get(peerOnion) ?: return
 // A name the user has chosen locally wins; only fill in one that was never set.
        val existing = contact.displayName.trim()
        if (existing.isNotBlank() && existing != peerOnion.take(8)) return
        storage.contacts.upsert(contact.copy(displayName = name))
        storage.conversations.get(peerOnion)?.let { conversation ->
            if (conversation.type == ConversationType.DIRECT) {
                storage.conversations.upsert(conversation.copy(title = name))
            }
        }
    }

    /** Binds a raw attachment transfer to the key from its offer. Both ends can compute it;
     * anyone who only observed the attachment id cannot. */
    private fun attachmentTransferToken(encKey: ByteArray, attachmentId: String): ByteArray =
        MessageDigest.getInstance("SHA-256").digest(encKey + attachmentId.toByteArray(Charsets.UTF_8))

    /**
     * The per-attachment key, or null if the row has gone.
     *
     * On purpose nullable rather than falling back to an empty array. An empty key still
     * produces a valid-looking token, one the recipient can never match, so the transfer
     * would fail on the far side with no indication why, and only for attachments whose row had
     * disappeared. A missing row is a real fault; it should stop the send and say so.
     */
    private fun encKeyForAttachment(attachmentId: String): ByteArray? =
        storage.attachments.get(attachmentId)?.encKey

    /** The context an attachment's ciphertext is bound to. Its id, which is unique and already
     * validated, so a file cannot be decrypted as a different attachment. */
    private fun attachmentContext(attachmentId: String): ByteArray =
        attachmentId.toByteArray(Charsets.UTF_8)

    /**
     * Decrypts an attachment.
     *
     * The attachment id is required as associated data, with no fallback. Ciphertext that does
     * not authenticate under its own id is refused rather than retried unbound, a fallback
     * would mean an attacker who could strip or alter the binding gets the unbound path for
     * free, which is the property the binding exists to prevent.
     *
     * Attachments created before the binding therefore no longer open. That is a deliberate
     * trade made while the only users are testers: accepting permanent weakening of every future
     * attachment to preserve a handful of old test files would be the wrong way round.
     */
    private fun decryptAttachmentToBytes(key: ByteArray, attachmentId: String, encFile: File): ByteArray? =
        runCatching {
            val out = ByteArrayOutputStream()
            encFile.inputStream().use { input ->
                storage.attachmentCipher.decrypt(key, input, out, attachmentContext(attachmentId))
            }
            out.toByteArray()
        }.getOrElse {
            retireUndecryptable(attachmentId, encFile)
            null
        }

    /** As above, to a file. */
    private fun decryptAttachmentToFile(
        key: ByteArray,
        attachmentId: String,
        encFile: File,
        dest: File,
 ...
 // A partial write may already exist; it is not the file the caller asked for.
        runCatching { dest.delete() }
        retireUndecryptable(attachmentId, encFile)
        false
    }

    private fun retireUndecryptable(attachmentId: String, encFile: File) {
        runCatching {
            DiagnosticLog.record(
                "attachment",
                "removed an attachment that predates the id binding and can no longer be decrypted",
            )
            encFile.delete()
 ...
 // the problem by passing an unvalidated value.
    ...
    suspend fun setSelfProfile(bytes: ByteArray?) = withContext(Dispatchers.IO) {
        ...
        notifyChanged()
        broadcastSelfProfile(bytes)
    }

    /**
     * Pushes this device's picture and display name to every direct contact.
     *
     * The name is sent alongside the picture rather than being local-only. Without it a contact
     * has nothing to show but the first few characters of an onion address, which is unusable
     * for telling people apart. Names are not unique or claimed: two people may
     * choose the same one, and that changes nothing, because identity comes from the onion
     * address and identity key. A name is a label for a human to read, never something the app
     * makes a trust decision on.
     */
    private suspend fun broadcastSelfProfile(bytes: ByteArray?) {
        val name = runCatching { username() }.getOrNull()?.takeIf { it.isNotBlank() }
        for (conversation in storage.conversations.list().filter { it.type == ConversationType.DIRECT }) {
            runCatching {
                ensureSession(conversation.id)
                sendWire(conversation.id, WireMessage.ProfileUpdate(selfOnion(), bytes, null, name))
            }
        }
    }
 ...
 // Only our own outgoing message may be recalled. remoteDeletable alone was not a real
 // check: incoming messages are stored with it set to true, so the flag permitted asking
 // everyone to delete a message somebody else wrote. Ownership is what actually
 // authorises a recall, and it is enforced here rather than only on the receiving side.
        if (existing == null) return@withContext
        if (existing.direction != MessageDirection.OUTGOING || existing.senderOnion != selfOnion()) {
            DiagnosticLog.record("send", "refused delete-for-everyone: not our own outgoing message")
            return@withContext
        }
        if (!existing.remoteDeletable) return@withContext
 ...
 // Validated before anything else, including before queueing. fromOnion is asserted by the
 // sender and libsignal will accept any string as an address name, so nothing upstream
 // guarantees this looks like an onion address. It flows into conversation ids, database
 // keys and filesystem paths, so a value like "../../databases/niix.db" must never get
 // that far, and queueing an unvalidated address would mean storing it on disk and only
 // rejecting it after unlock, having already spent the space.
 ...
 // Enforced before decryption or any database write, so an authenticated flood is cheap
 // to refuse. Returning false rather than true means the sender treats it as undelivered
 // and backs off or relays, which is the correct signal, claiming success would
 // silently discard messages a legitimate peer believed had arrived.
        if (!senderWithinQuota(fromOnion, ciphertext.size)) {
            DiagnosticLog.record("receive", "sender exceeded its message quota -- refused")
            return@withContext false
        }

 // While locked the database is closed, so the Signal session store this message needs is
 // unavailable and it genuinely cannot be processed now. Queue it instead of failing:
 // the sender gets a successful delivery, the message is held as the ciphertext it
 // already is, and it is processed the moment the app unlocks. Nothing is decrypted or
 // written to the database while locked.
        if (!storage.appLock.isUnlocked()) {
            val queued = storage.lockedInbox.enqueue(fromOnion, ciphertext, viaRelay)
            if (!queued) {
 // Full or unwritable, refuse rather than pretend. The sender will retry or
 // fall back to the relay, which is the correct outcome.
                DiagnosticLog.record("receive", "locked inbox full -- message refused, sender will retry")
                return@withContext false
            }
 // Tell the user something arrived. Nothing here has been decrypted, the fact that
 // a message was received is known from the connection itself, not its contents. The
 // notification is the same generic one shown when unlocked (no sender, no preview,
 // and titled as the calculator when the disguise is on), so a locked device reveals
 // no more than an unlocked one does.
 // Emitted directly rather than via emitIncoming, which looks up the conversation
 // title in the database, closed while locked, so that would throw and take the
 // whole queueing path down with it. The title is unused anyway: the notification
 // shown is generic.
            _incoming.tryEmit(IncomingNotice(fromOnion, fromOnion.take(12), ""))
            return@withContext true
        }
 ...
 // it, and without a record the message would simply vanish with no trace.
                ...
                storage.messages.insert(
                    incomingMessage(convId, wire.messageId, fromOnion, MessageType.TEXT, wire.body, null, wire.expiresSeconds)
                        .copy(replyToId = wire.replyToId),
                )
 ...
 // attachments into it.
                ...
                // Exact lengths, not merely present.
                //
                // The key and digest are read with the generic block limit, so a sender could
                // offer a 64 KB "digest" and this device would store it. Nothing would notice
                // until decryption failed much later, by which point the junk is in the database
                // and the failure looks like corruption rather than a malformed offer. Both are
                // fixed-size by definition, so checking costs nothing and turns a stateful
                // failure into an immediate rejection.
                if (wire.digest == null || wire.digest.size != ATTACHMENT_DIGEST_BYTES) return
                if (wire.encKey.size != ATTACHMENT_KEY_BYTES) return
 ...
 // Authorisation requires all of: the message exists, it belongs to the
 // conversation this request names, the authenticated sender wrote it, and that
 // sender is still a party to that conversation.
 //
 // Ownership alone was enough to stop the obvious attack, but it left the
 // predicate incomplete: a request could name one conversation while targeting a
 // message in another, and a member removed from a group could still delete
 // messages they had written there. Checking the full set means the request has
 // to be coherent, not merely point at something the sender once wrote.
                val claimedConversation = resolveIncomingConversationId(wire.conversationId, fromOnion)
                wire.targetMessageIds.forEach { messageId ->
                    val message = storage.messages.get(messageId)
                    val authorized = message != null &&
                        message.conversationId == claimedConversation &&
                        message.senderOnion == fromOnion &&
                        isAuthorizedForGroupIfGroup(claimedConversation, fromOnion)
                    if (authorized) {
                        storage.messages.markDeletedForEveryone(messageId)
                    } else if (message != null) {
                        DiagnosticLog.record("receive", "refused delete-for-everyone: request not authorized for that message")
                    }
                }
            }
            ...
            is WireMessage.Receipt -> {
                val message = storage.messages.get(wire.messageId) ?: return
                if (!isAuthorizedForConversation(message.conversationId, fromOnion)) return
                if (wire.state == READ_RECEIPT_STATE) {
                    // Only for messages this device sent.
                    //
                    // A read receipt says "I have read what you sent me", so it can only
                    // legitimately start the clock on the sender's own copy. Applying it to an
                    // incoming message let any group member expire a message on someone else's
                    // device: Carol sends a forged READ for Alice's message and Bob's copy starts
                    // disappearing without Bob ever opening it. Nothing else here caught that,
                    // because Carol is a genuine member and the receipt is genuinely from her.
                    //
                    // An incoming message's timer starts when this device actually reads it,
                    // which markMessageRead handles.
 ...
 // could verify them. See GroupMembershipPolicyTest.
                    val decision = GroupMembershipPolicy.evaluate(
                        ...
                        sender = fromOnion,
                        currentStateHash = currentGroupStateHash(wire.conversationId),
                        claimedPrevStateHash = wire.prevStateHash,
                        chainEstablished = isChainEstablished(wire.conversationId),
                    )
                    if (decision is GroupMembershipPolicy.Decision.Reject) {
                        DiagnosticLog.record("receive", "rejected group update: ${decision.reason}")
                    }
 ...
 // Every member of this group is now demonstrably on a build that chains
 // state, so the compatibility path can be closed for it permanently.
                        if (wire.prevStateHash != null) markChainEstablished(wire.conversationId)
                        rememberGroupState(wire.conversationId)
                        notifyChanged()
                    }
                }
            }
            is WireMessage.ProfileUpdate -> {
                ...
                if (groupId == null) {
                    ...
                    applyPeerDisplayName(fromOnion, wire.displayName)
                    notifyChanged()
            ...
            is WireMessage.RemoteWipe -> {
 // Handled here, in dispatch, alongside receipts and group updates, so it is
 // never stored as a message, never displayed and never notified. That is also
 // why blurring is irrelevant to it: there is nothing to render.
 //
 // Nothing is sent back, whether it fires or not. An acknowledgement would tell
 // the sender this device exists, is reachable and was running the app; a refusal
 // would tell an attacker their token was wrong, which is a guessing oracle.
 // Silence is the only safe response in both directions.
                if (storage.remoteWipe.shouldWipe(fromOnion, crypto.remoteIdentityKeyBytes(fromOnion))) {
                    DiagnosticLog.record("wipe", "remote wipe accepted from a nominated contact")
                    onRemoteWipe?.invoke()
                } else {
 // Someone attempting this is itself worth knowing, even though they get no
 // response.
                    DiagnosticLog.record("wipe", "remote wipe REFUSED (not nominated, or wrong token)")
                }
            }
            is WireMessage.RemoteWipeNomination -> {
 // Recorded only for an existing contact. An unsolicited nomination from a
 // stranger would put them in the user's "wipe another device" list, which is
 // confusing at best and at worst invites wiping a device that never asked.
                // Only from a contact whose identity key is pinned.
                //
                // This list decides who appears in "wipe another device", so an entry here is an
                // invitation to destroy that person's data. An unverified contact is
                // trust-on-first-use, and accepting their claim would let anyone who reached the
                // user once put themselves on a destructive list. Withdrawal is always accepted:
                // removing a permission must never be harder than granting it.
                val trusted = wire.nominated == false ||
                    (storage.contacts.isKnown(fromOnion) && identityMatchesPinnedKey(fromOnion))
                if (trusted) {
                    storage.remoteWipe.setCanWipe(fromOnion, wire.nominated)
                    DiagnosticLog.record(
                        "wipe",
                        if (wire.nominated) "a contact nominated this device to erase theirs"
                        else "a contact withdrew that nomination",
                    )
                }
            }
            is WireMessage.GroupLeave -> {
                val conversation = storage.conversations.get(wire.conversationId)
 // Only a current member can leave, and they can only ever remove themselves --
 // fromOnion is authenticated by the session, and nothing here reads a membership
 // list, so this transition cannot be used to affect anyone else.
                if (conversation != null && conversation.type == ConversationType.GROUP &&
                    storage.members.isMember(wire.conversationId, fromOnion) &&
                    wire.epoch > conversation.epoch
                ) {
                    val remaining = storage.members.listForConversation(wire.conversationId)
                        .filter { it.memberOnion.value != fromOnion }
 // If the departing member was the last admin, promote deterministically --
 // the lowest-sorted remaining member. Every device computes the same answer
 // from state it already holds, so nobody has to trust the leaver's word for
 // who inherits authority.
                    val remainingAdmins = remaining.filter { it.role == GroupRole.ADMIN }
                        .map { it.memberOnion.value }
                    val admins = remainingAdmins.ifEmpty {
                        listOfNotNull(remaining.map { it.memberOnion.value }.minOrNull())
                    }
                    storage.conversations.upsert(conversation.copy(epoch = wire.epoch))
                    storage.members.replaceAll(
                        wire.conversationId,
                        remaining.map { it.memberOnion.value },
                        admins,
                    )
                    revokeSenderKeysFor(wire.conversationId, listOf(fromOnion))
                    rememberGroupState(wire.conversationId)
                    notifyChanged()
                } else {
                    DiagnosticLog.record("receive", "dropped group leave: not a current member, or stale epoch")
                }
            }
            ...
            is WireMessage.GroupCiphertext -> {
                // Both failures below throw rather than return. A plain return let handleIncoming
                // acknowledge the message, so the sender marked it delivered while this phone had
                // thrown it away. Throwing makes handleIncoming report failure instead: the sender
                // keeps the message pending and retries, with this device's key sent first.
                if (!isAuthorizedForConversation(wire.conversationId, fromOnion)) {
                    // Usually a membership update that has not reached this phone yet.
                    DiagnosticLog.record("receive", "group message from a sender not yet known as a member -- not acknowledged, sender will retry")
                    throw IllegalStateException("group sender not authorised")
                }
                ...
                if (inner == null) {
                    // Almost always the sender's group key has not arrived yet.
                    DiagnosticLog.record("receive", "group message could not be decrypted -- not acknowledged, sender will retry with its key")
                    throw IllegalStateException("group message undecryptable")
                }
                dispatch(fromOnion, inner)
            }
    ...
    private suspend fun deliverToConversation(conversation: Conversation, wire: WireMessage): Boolean {
        return when (conversation.type) {
            ...
            ConversationType.GROUP -> deliverToGroup(conversation, wire)
        }
    }

    /**
     * Sends a group message to every member, and reports success only once all of them have it.
     *
     * This used to count a message as delivered as soon as any one member acknowledged it. Every
     * member who was unreachable at that moment was then never tried again, and group sends had no
     * relay fallback either, so whether a message reached a phone came down to whether that phone
     * happened to be online when it was sent.
     *
     * Now each member is tracked separately. The members still owed a message are stored under the
     * message id, a retry sends only to them, and the message stays pending until the list is
     * empty. A member who leaves drops off the list; someone who joins later is not added, so they
     * do not receive messages from before they joined. Sends fall back to a relay like direct
     * messages do, and a relayed copy still counts as owed until the member acknowledges directly,
     * the same as a one-to-one message.
     *
     * Each member also receives this device's group key before the message, unless they are known
     * to hold it already, and always on a retry. The key used to be sent once, when it was
     * created; a member who missed that copy could never decrypt anything from this sender, and
     * their phone discarded those messages silently.
     */
    private suspend fun deliverToGroup(conversation: Conversation, wire: WireMessage): Boolean {
        val members = storage.members.listForConversation(conversation.id)
            .map { it.memberOnion.value }
            .filter { it != selfOnion() }
        if (members.isEmpty()) return false

        val trackingKey = groupTrackingId(wire)?.let { pendingRecipientsKey(it) }
        val stored = trackingKey?.let { storage.settings.getString(it) }
        val firstAttempt = stored == null
        val remaining = if (stored == null) members else stored.split('\n').filter { it.isNotBlank() && it in members }
        if (remaining.isEmpty()) {
            trackingKey?.let { storage.settings.remove(it) }
            return true
        }

        // Only content is wrapped in the group key; control messages go pairwise as before.
        val keyState = if (wire is WireMessage.Text || wire is WireMessage.AttachmentOffer) {
            ensureGroupSenderKey(conversation)
        } else {
            null
        }
        // Taken before encrypting, so it describes the key at or before this message's position.
        // A key message from a later position cannot decrypt an earlier message.
        val keyWire = keyState?.let { state ->
            runCatching {
                WireMessage.SenderKeyDistribution(
                    conversation.id,
                    selfOnion(),
                    crypto.groupDistributionFor(state.distributionId),
                )
            }.getOrNull()
        }
        val outgoing = if (keyState != null) groupWrapIfApplicable(conversation, wire) ?: wire else wire
        val holdersKey = keyState?.let { keyHoldersKey(conversation.id, it.distributionId) }
        val holders = holdersKey?.let { readOnionSet(it) } ?: emptySet()

        val stillOwed = mutableListOf<String>()
        val newHolders = mutableSetOf<String>()
        for (onion in remaining) {
            runCatching { ensureSession(onion) }
            if (keyWire != null && (!firstAttempt || onion !in holders)) {
                val keyOutcome = try {
                    sendWireDirectOrRelay(onion, keyWire)
                } catch (_: Exception) {
                    DeliveryOutcome.FAILED
                }
                // Without the key the message cannot be read, and if the member is locked their
                // phone would accept it now and fail to open it later. So no key, no message:
                // they stay owed and get both next time.
                if (keyOutcome == DeliveryOutcome.FAILED) {
                    stillOwed += onion
                    continue
                }
            }
            val outcome = try {
                sendWireDirectOrRelay(onion, outgoing)
            } catch (_: Exception) {
                DeliveryOutcome.FAILED
            }
            if (outcome == DeliveryOutcome.DELIVERED) {
                // A direct acknowledgement of a group message means it was decrypted, so this
                // member provably holds the key and does not need it resent on first attempts.
                if (keyState != null) newHolders += onion
            } else {
                stillOwed += onion
            }
        }

        if (holdersKey != null && newHolders.isNotEmpty()) {
            writeOnionSet(holdersKey, holders + newHolders)
        }
        if (trackingKey != null) {
            if (stillOwed.isEmpty()) {
                storage.settings.remove(trackingKey)
            } else {
                storage.settings.setString(trackingKey, stillOwed.joinToString("\n"))
            }
        }
        if (stillOwed.isNotEmpty()) {
            DiagnosticLog.record("send", "group message still owed to ${stillOwed.size} of ${members.size} member(s); will retry")
        }
        return stillOwed.isEmpty()
    }

    /** Messages with an id can be tracked per member; control messages have none and are not. */
    private fun groupTrackingId(wire: WireMessage): String? = when (wire) {
        is WireMessage.Text -> wire.messageId
        is WireMessage.AttachmentOffer -> wire.messageId
        else -> null
    }

    private fun pendingRecipientsKey(messageId: String) = "grouppending:$messageId"

    private fun keyHoldersKey(conversationId: String, distributionId: String) =
        "groupkeyholders:$conversationId:$distributionId"

    private fun readOnionSet(key: String): Set<String> =
        storage.settings.getString(key)?.split('\n')?.filter { it.isNotBlank() }?.toSet() ?: emptySet()

    private fun writeOnionSet(key: String, onions: Set<String>) {
        storage.settings.setString(key, onions.joinToString("\n"))
    }
    ...
    private fun groupStateHashKey(conversationId: String) = "groupstate:$conversationId"

    private fun currentGroupStateHash(conversationId: String): String? =
        storage.settings.getString(groupStateHashKey(conversationId))

    /** Set once a group has accepted a chained transition, and never cleared, after that,
     * unchained transitions for that group are refused. See GroupMembershipPolicy. */
    private fun groupChainKey(conversationId: String) = "groupchain:$conversationId"

    private fun isChainEstablished(conversationId: String): Boolean =
        storage.settings.getString(groupChainKey(conversationId)) == "1"

    private fun markChainEstablished(conversationId: String) {
        storage.settings.setString(groupChainKey(conversationId), "1")
    }

    /** Records the state now in force, after a transition has been applied locally. */
    private fun rememberGroupState(conversationId: String) {
        val conversation = storage.conversations.get(conversationId) ?: run {
 // No baseline recorded means the next transition has nothing to be checked against,
 // so the state chain quietly degrades to accepting anything with a higher epoch.
 // That is the bypass the chain exists to prevent, so it should never happen
 // unnoticed even though it should never happen at all.
            DiagnosticLog.record("group", "could not record state baseline: conversation missing")
            return
        }
        val members = storage.members.listForConversation(conversationId)
        val hash = GroupMembershipPolicy.stateHash(
            conversationId = conversationId,
            epoch = conversation.epoch,
            members = members.map { it.memberOnion.value },
            admins = members.filter { it.role == GroupRole.ADMIN }.map { it.memberOnion.value },
        )
        storage.settings.setString(groupStateHashKey(conversationId), hash)
    }
        ...
        try {
            ...
            when (type) {
                FRAME_ATTACHMENT -> {
                    ...
                    if (tokenLen !in 1..64) return@withContext
 ...
 // Verified before a single byte is written, so an unauthenticated blob costs
 // nothing and cannot disturb a transfer that is legitimately in progress.
                    val expectedToken = attachmentTransferToken(pending.encKey, attachmentId)
                    if (!MessageDigest.isEqual(token, expectedToken)) {
                        DiagnosticLog.record("receive", "dropped attachment blob: transfer token did not match")
                        return@withContext
                    }
    ...
    private suspend fun ensureSession(peerOnion: String) {
        ...
        runCatching {
 // Sent even when there is no picture, because the display name alone is worth
 // delivering, otherwise a contact who never set a picture stays permanently
 // nameless to everyone they talk to.
            val self = loadProfile("self")
            val name = runCatching { username() }.getOrNull()?.takeIf { it.isNotBlank() }
            if (self != null || name != null) {
                sendWire(peerOnion, WireMessage.ProfileUpdate(selfOnion(), self, null, name))
            }
        }
 ...
 // No pin at all is ordinary trust-on-first-use and stays permitted.
        val pinned = storage.settings.getString(identityKeyStoreKey(peerOnion)) ?: return true

 // From here on, fail closed. A pin exists because the user explicitly scanned this
 // contact's code, so every remaining outcome is either a match or a state we cannot
 // vouch for. Returning true on a corrupt pin or a missing identity meant the strongest
 // signal a user can give, "I verified this person in person", was silently ignored
 // precisely when something was already wrong.
        val expected = runCatching { Base64.getDecoder().decode(pinned) }.getOrNull()
        if (expected == null) {
            DiagnosticLog.record("receive", "pinned identity for this contact is unreadable -- refusing")
            return false
        }
        val actual = crypto.remoteIdentityKeyBytes(peerOnion)
        if (actual == null) {
            DiagnosticLog.record("receive", "no stored identity to compare against a pin -- refusing")
            return false
        }
    ...
    private fun newOutgoingMessage(
        conversation: Conversation,
        type: MessageType,
        body: String,
        attachmentId: String?,
     ...
     *
     * On purpose different from [isAuthorizedForConversation]: that one refuses an unknown
 ...
 // Record the peer as a contact as well as opening the conversation.
 //
 // Relay grants are only issued to contacts, and a message can only be relayed to
 // someone who has issued you one. Creating just a conversation left the person who
 // did not scan the code absent from the other side's contact list, so no grant was
 // ever issued to them, and their replies could only be delivered while the peer
 // happened to be directly reachable. That is why the direction that scanned worked
 // and the direction that was scanned did not, and why scanning back appeared to fix
 // it: it finally created the contact row this was missing.
 //
 // UNVERIFIED, because nothing here establishes who they are, it records that a
 // conversation exists, which is what grant issuance needs, not a trust decision.
            if (isStranger) {
                storage.contacts.upsert(
                    Contact(
                        onionAddress = OnionAddress.parse(fromOnion),
                        displayName = fromOnion.take(8),
                        fingerprint = IdentityFingerprint(""),
                        trustState = TrustState.UNVERIFIED,
                        addedAtEpochMillis = now(),
                    ),
    ...
    /**
     * Local time for records this device creates.
     *
     * Monotonic, so winding the device clock back does not push expiry deadlines into the past or
     * make new messages sort before old ones. Timestamps arriving from other devices are used as
     * they are and never go through here: clamping what a sender said would misrepresent it.
     */
    private fun now(): Long = storage.clock.now()

    companion object {
        /** Past this many tracked senders, idle limiters are cleared out. */
        private const val LIMITER_SWEEP_THRESHOLD = 256
        ...
        /** SHA-256. */
    private const val ATTACHMENT_DIGEST_BYTES = 32

    /** The streaming AEAD key size AttachmentCipher generates. */
    private const val ATTACHMENT_KEY_BYTES = 32

    private const val MAX_ATTACHMENT_BYTES = 100L * 1024 * 1024
        private const val MAX_ONION_BYTES = 256

        /** Display names come from a remote peer and are shown in lists and notifications, so
         * they are capped rather than trusted to be a sensible length. */
        private const val MAX_DISPLAY_NAME_CHARS = 40

        /** Generous for real conversation, far above any human typing rate, while bounding
         * what a hostile peer can accumulate. */
        private const val MAX_MESSAGES_PER_SENDER_PER_MINUTE = 120L
        private const val MAX_BYTES_PER_SENDER_PER_MINUTE = 16L * 1024 * 1024
