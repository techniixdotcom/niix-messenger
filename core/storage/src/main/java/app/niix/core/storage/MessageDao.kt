...
class MessageDao internal constructor(private val secureDatabase: SecureDatabase) {
    ...
    private fun android.database.Cursor.toMessage(): Message {
        ...
        // getColumnIndex, not OrThrow: a database created before this column existed has it
        // added by migration, but reading must not hard-fail if that has not run yet.
        val replyIdx = getColumnIndex(t.COL_REPLY_TO_ID)
        val revealedIdx = getColumnIndex(t.COL_REVEALED)
        return Message(
            ...
            replyToId = if (replyIdx < 0 || isNull(replyIdx)) null else getString(replyIdx),
            revealed = revealedIdx >= 0 && getInt(revealedIdx) == 1,
        )
    }
}
