...
class ContactDao internal constructor(private val secureDatabase: SecureDatabase) {
    ...
    /**
     * A contact by address, matched case-insensitively.
     *
     * Onion addresses are base32 and case-insensitive by specification, so the same contact can
     * be written two ways. An exact match meant a lookup silently returned nothing when the
     * stored form differed from the one being searched for, and callers show an address prefix
     * when no contact is found -- so a group showed everyone as unnamed addresses while the
     * contacts were sitting in the database.
     */
    fun get(onion: String): Contact? {
        db.rawQuery(
            "SELECT * FROM ${t.TABLE} WHERE ${t.COL_ONION} = ? COLLATE NOCASE",
            arrayOf(onion),
        ).use { c ->
