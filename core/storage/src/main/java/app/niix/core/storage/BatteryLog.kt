...
class BatteryLog internal constructor(private val settings: SettingsStore) {
    ...
    fun render(): String {
        ...
        // Still connected at the end of the log, which is the normal case when nothing ever
        // disconnected. Without this the total only counted completed connect/disconnect pairs,
        // so a run that stayed connected the whole time reported 0% and looked like the app had
        // never connected at all, which is the opposite of what happened.
        if (lastUp != null) connectedMillis += all.last().first - lastUp
        ...
        val starts = all.count { it.second.startsWith("service started") }
        val connects = all.count { it.second.startsWith("connected") }
        if (starts > connects) {
            sb.append("\n  $starts service starts produced $connects connection(s): the rest found it already running\n")
        }
