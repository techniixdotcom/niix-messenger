...
class EncryptedBackup internal constructor(
    ...
    /**
     * Exports the database into an AEAD-encrypted backup file. SQLCipher's export routine can
     * only copy into another *attached* database, so we attach one keyed with a random,
     * single-use key -- never an empty/plaintext key -- so the intermediate file on disk is
     * never unencrypted, even momentarily. That ephemeral key travels inside the final
     * passphrase-encrypted payload so import() can recover it.
     */
        ...
        try {
            ...
            try {
                destination.outputStream().use { rawOut ->
                    rawOut.write(header)
                    // The AEAD-protected stream is [ephemeral key][SQLCipher-encrypted export].
                    // The export bytes are already ciphertext, so this is double-encrypted --
                    // no plaintext database content ever touches disk during export.
    ...
    /**
     * Imports a backup produced by [export]. The decrypted stream is split as it's written: the
     * first [EPHEMERAL_KEY_BYTES] bytes (the ephemeral key) are captured in memory, and only the
     * remaining SQLCipher-encrypted bytes are written to disk -- so the export never exists as
     * plaintext on disk here either.
     */
    ...
    /**
     * Routes the first [headBytes] bytes written to [head] and everything after to [tail],
     * correctly handling a single write() call whose bytes straddle that boundary (the
     * streaming decryptor writes in large chunks, not one byte at a time).
     */
