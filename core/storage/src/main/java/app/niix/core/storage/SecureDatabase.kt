...
class SecureDatabase internal constructor(private val appContext: Context) {
 ...
 // missing columns silently becoming missing authorization data.
    ...
    /**
     * Undoes a rekey that was interrupted by the process dying.
     *
     * The marker is only present when a rekey started and did not verify, so its presence means
     * the database may have been re-encrypted under a passphrase that was never confirmed to
     * work, leaving the user unable to unlock with either the old or the new one. Restoring
     * the backups returns them to a database their existing passcode opens, which is the only
     * recoverable outcome. A passcode change that has to be repeated is a nuisance; a database
     * nobody can open is the whole history gone.
     *
     * Runs before any open attempt, so recovery happens before anything reads the files.
     */
