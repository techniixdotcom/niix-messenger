...
class EncryptedFileStore internal constructor(context: Context) {
    ...
    private fun resolve(name: String): File {
        // A positive character set, not a list of things to reject. Traversal is already caught
        // below, but "everything except the attacks I thought of" is a weaker rule than "these
        // characters only": it has to be revisited every time someone finds another separator,
        // encoding or reserved name that a filesystem treats specially.
        require(name.isNotBlank()) { "Empty file store name" }
        require(name.length <= MAX_NAME_LENGTH) { "File store name too long" }
        require(name.all { it.isLetterOrDigit() || it == '-' || it == '_' || it == '.' }) {
            "File store name must be letters, digits, dash, underscore or dot"
        }
 ...
 // outside the store, which is if anything worse than writing them.
    ...
    companion object {
        ...
        /** Comfortably under every common filesystem limit, and far longer than any name this
         * store actually uses. */
