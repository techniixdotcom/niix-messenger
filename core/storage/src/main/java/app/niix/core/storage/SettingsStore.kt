...
class SettingsStore internal constructor(private val secureDatabase: SecureDatabase) : KeyValueStore {
    ...
    companion object {
        ...
        /** Minutes between wakeups while the connection is down. See SleepPoller. */
        const val KEY_SLEEP_POLL_MINUTES = "sleep_poll_minutes"

        /** Whether to drop the connection (and its notification) while the screen is off. */
        const val KEY_SLEEP_WHEN_IDLE = "sleep_when_idle"

        /** Whether hosting a relay keeps the connection up while the screen is off. */
        const val KEY_RELAY_KEEPS_CONNECTION = "relay_keeps_connection"

        /** Opt-in battery measurement. See BatteryLog. */
        const val KEY_BATTERY_LOG_ENABLED = "battery_log_enabled"
        const val KEY_BATTERY_LOG = "battery_log"
