...
class MemberDao internal constructor(private val secureDatabase: SecureDatabase) {
    ...
    /** Is [onion] still a member of any group at all? Used to decide whether removing them from
     * one group means every sender key we hold from them can be dropped, or whether they remain
     * a participant elsewhere and their keys are still needed. */
    fun isInAnyGroupWith(onion: String): Boolean {
        db.rawQuery(
            "SELECT 1 FROM ${t.TABLE} WHERE ${t.COL_MEMBER_ONION} = ? LIMIT 1",
            arrayOf(onion),
        ).use { c -> return c.moveToFirst() }
    }

    fun isMember(conversationId: String, onion: String): Boolean {
        db.rawQuery(
            "SELECT 1 FROM ${t.TABLE} WHERE ${t.COL_CONVERSATION_ID} = ? AND ${t.COL_MEMBER_ONION} = ? LIMIT 1",
            arrayOf(conversationId, onion),
        ).use { c -> return c.moveToFirst() }
    }
