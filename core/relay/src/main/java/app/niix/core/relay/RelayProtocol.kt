...
object RelayProtocol {
    ...
    /**
     * Shortest lifetime a relayed envelope may be given.
     *
     * A TTL of zero was previously accepted and stored with an expiry of "now", so the envelope
     * was discarded the instant it was written while the sender was told it had been stored --
     * a silent message-loss path reachable by a buggy or hostile sender. An envelope worth
     * relaying is worth holding long enough for the recipient to plausibly fetch it.
     */
    const val MIN_RELAY_TTL_MILLIS = 5L * 60 * 1000
