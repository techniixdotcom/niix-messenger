...
class KademliaLookup(
 ...
 // Only the responder enters the routing table, and only because RelayClient
 // verified its self-signed node identity before constructing it. This is the
 // invariant that bounds DHT poisoning: a hostile relay can name any nodes it
 // likes in its response, but naming them cannot make them persist. Inserting
 // candidates here would hand an attacker durable control of routing state --
 // so if this ever needs to change, the candidates must carry and pass their own
 // identity proofs first.
                if (result.responder != null) {
                    routingTable.insertOrUpdate(result.responder)
                }
 // Candidates are unauthenticated hints, used only to widen this one lookup.
 // Anything actually contacted has to prove its identity before it is kept, so a
 // poisoned hint costs a wasted connection rather than a corrupted routing table.
