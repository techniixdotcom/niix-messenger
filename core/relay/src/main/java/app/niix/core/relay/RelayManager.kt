...
class RelayManager(
    ...
    private val sweeper: RelaySweeper = RelaySweeper(store, routingTable, rateLimiter)
    ...
    /**
     * Sets how much this device will store for others.
     *
     * Zero turns hosting off rather than leaving it on with nowhere to put anything. Previously a
     * budget of zero kept hosting enabled, so the app announced itself as a relay, accepted
     * connections and then rejected every envelope. Peers would keep choosing it and keep
     * failing, and the user's settings would say relaying was on while it did nothing.
     */
    fun setStorageBudgetBytes(bytes: Long) {
        ...
        if (clamped == 0L && isHostingEnabled()) {
            DiagnosticLog.record("relay", "storage budget set to zero: hosting turned off")
 ...
 // and this is the path that decides whether an offline peer gets a message at all.
     ...
     *
     * The delete is not performed here. It used to be, immediately after fetching --
    ...
    suspend fun fetchIncoming(): List<FetchedEnvelope> {
        ...
        for (node in candidates) {
            // A separate proof per relay, binding that relay's address into what is signed.
            //
            // One proof reused across relays meant any relay that saw it could present the same
            // bytes to the others and collect the same envelopes. The local replay cache stops
            // that at the relay that issued it and does nothing about the rest. Signing the
            // relay's address makes a proof useless anywhere else: there is nothing to replay,
            // rather than a replay that gets caught.
            val proofSig = RelaySigning.signFetchProof(crypto, localIdKey, now, node.onion)
    ...
    suspend fun confirmProcessed(fetched: FetchedEnvelope) {
        val localIdKey = crypto.localIdentityKey()
        val deleteSig = RelaySigning.signDeleteProof(crypto, localIdKey, fetched.envelopeHash, fetched.nodeOnion)
