...
class RelayManager(
    ...
    /**
     * Records that an existing contact has turned relay hosting on or off.
     *
     * Inserts into the routing table without a self-signed node proof, unlike every other path,
     * and that is correct here rather than an omission: this is only reachable from a decrypted
     * message over an established Signal session, so the onion address is the authenticated
     * sender and the identity key comes from the identity store. The proof used elsewhere exists
     * to authenticate strangers; a contact has already been authenticated more strongly.
     *
     * It must stay that way. If this is ever called from anywhere the sender is not already
     * authenticated, it becomes an unauthenticated write into persistent routing state.
     */
