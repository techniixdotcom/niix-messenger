...
class RelayConnectionHandler(
    ...
    /** A proof is honoured once. See ProofReplayCache. */
    private val proofReplay = ProofReplayCache(RelayProtocol.RELAY_FETCH_PROOF_WINDOW_MS)
    ...
    private fun handleStore(input: DataInputStream, output: OutputStream) {
        ...
        // elapsedRealtime, not `now`. `now` is wall-clock because envelope TTLs are compared
        // against timestamps from other devices, but the rate limiter measures a local duration
        // and the two clocks must not be mixed inside it.
        if (!rateLimiter.allow(senderIdKey, android.os.SystemClock.elapsedRealtime())) {
    ...
    private fun handleFetch(input: DataInputStream, output: OutputStream) {
        ...
        // Verified against this relay's own address, which the client signed. A proof made for a
        // different relay will not verify here, so it cannot be replayed across the network.
        val selfOnion = selfOnionProvider()
        if (selfOnion == null) {
            reject(output, RelayRejectReason.PROOF_INVALID)
            return
        }
        val proofValid = RelaySigning.verifyFetchProof(
            crypto,
            claimedIdentityKey = recipientIdKey,
            timestampMillis = timestamp,
            relayOnion = selfOnion,
            signature = proofSig,
        )
        if (!proofValid) {
            reject(output, RelayRejectReason.PROOF_INVALID)
 ...
 // Checked after the signature, so an invalid proof never occupies a cache slot, an
 // attacker sending garbage could otherwise fill the cache rather than being rejected
 // outright.
        if (!proofReplay.offer(proofSig, now)) {
            reject(output, RelayRejectReason.PROOF_INVALID)
            return
        }
    ...
    private fun handleDeleteReceipt(input: DataInputStream) {
        ...
        val proofSig = readBlock(input, MAX_SIGNATURE_BYTES)
        val selfOnion = selfOnionProvider() ?: return
        val proofValid = RelaySigning.verifyDeleteProof(
            crypto,
            claimedIdentityKey = recipientIdKey,
 ...
 // A delete proof signs only the envelope hash, no timestamp, so unlike the fetch
 // proof it never expires. Deleting an already-deleted envelope is harmless, but the same
 // hash can legitimately be stored again later (a resend after expiry), and a captured
 // proof would delete that new copy. Honouring each proof once closes that.
        if (proofValid && proofReplay.offer(proofSig, System.currentTimeMillis())) {
