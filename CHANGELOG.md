...
- Lower battery use while idle: Tor now uses its reduced-padding modes for mobile
- Optional battery measurement in Settings > About, off by default, for diagnosing battery use
- Group messages now show who sent them
- Safety numbers can be checked per member from group info; the toolbar option no longer appears
  in groups, where it could never work
- Dialogs now actually use the app's dark surface (the previous attempt was silently ignored)
- Updated the Monero donation address
- Malformed attachment offers are now rejected on arrival instead of failing later
- Dialogs no longer crash the app
- Disappearing messages can no longer be kept alive by winding the device clock back
- Hosting a relay no longer forces the connection to stay up: there is now a choice, so battery
  saving and relaying can be combined
- Scheduled wakeups now wait for the connection instead of a fixed two minutes, so a slow start
  no longer means collecting nothing
- Examining the phone's files no longer reveals that a duress code is set or that the calculator
  is a disguise
- Group chats show contact names instead of address fragments
- Group chats no longer show an "unverified identity" warning: it pointed at an option that does
  not exist in groups and suggested a problem that was not there
