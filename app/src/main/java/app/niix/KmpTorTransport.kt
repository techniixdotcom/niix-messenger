...
class KmpTorTransport(
 ...
 // minutes is a moderate step, not the maximum, .
                TorOption.MaxCircuitDirtiness.configure(30, IntervalUnit.MINUTES)

                // Padding options Tor provides specifically for mobile. The manual says both
                // "should be offered via the UI to mobile users for use where bandwidth may be
                // expensive", and both default to off, so a phone pays full padding cost unless
                // asked otherwise.
                //
                // ReducedConnectionPadding stops Tor holding OR connections open for very long
                // and sends less padding on them. ReducedCircuitPadding restricts it to padding
                // algorithms with low overhead. Together they cut the traffic this app generates
                // while completely idle, which on a phone is radio wakeups and therefore battery.
                //
                // There is a real cost. Connection and circuit padding exist to blunt traffic
                // analysis, and reducing them makes this device's patterns somewhat easier to
                // fingerprint for an adversary watching the guard. It is a narrower loss here
                // than for browsing: onion-service traffic never leaves via an exit, so no relay
                // on the path sees a destination. Against the global passive adversary the
                // threat model already says NiiX does not defend against, this changes nothing.
                TorOption.ReducedConnectionPadding.configure(true)
                TorOption.ReducedCircuitPadding.configure(true)
            }
        }
    }
 ...
 // with no way for the app to show real progress in the meantime.
    ...
    private suspend fun awaitNetworkReady(timeoutMillis: Long = 120_000L) {
        // elapsedRealtime, not the wall clock. This measures a duration on this device, and the
        // wall clock can jump: a network time sync mid-bootstrap could expire the deadline
        // instantly or push it minutes into the future. Only timestamps that cross devices need
        // the wall clock; a local timeout does not.
 ...
 // defeats the point of having moved it into the encrypted database at all.
    ...
    override suspend fun stop() {
        ...
        // Only if nothing restarted the connection while the stop was in progress. A slow stop
        // finishing after a start would otherwise label a running connection as stopped.
        if (_state.compareAndSet(TransportState.STOPPING, TransportState.STOPPED)) {
 ...
 // A read deadline on every inbound socket. Without one, a connection that
 // opens and then never sends anything occupies a handler indefinitely, which is
 // what makes a slow-loris style exhaustion attack cheap: the attacker spends
 // almost nothing, and each connection costs this device a coroutine and a
 // socket until the process dies.
                runCatching { socket.soTimeout = INBOUND_READ_TIMEOUT_MILLIS }
    ...
    companion object {
        private const val INBOUND_READ_TIMEOUT_MILLIS = 30_000
