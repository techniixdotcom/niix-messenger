...
object NotificationHelper {
    ...
    fun postMessage(context: Context, title: String, text: String, conversationId: String) {
        ...
        // Under the calculator disguise the notification is titled as the calculator, so a
        // glance at the shade shows something consistent with the app the device appears to be
        // running rather than announcing a messenger.
        val disguised = runCatching {
            (context.applicationContext as NiixApp).container.storage.appLock.isDisguiseEnabled()
        }.getOrDefault(false)
        val shownTitle = if (disguised) context.getString(R.string.calc_label) else title
