...
/**
 * Draws [bitmap] with a fixed square crop frame overlaid; the person drags to reposition and
 * pinches to zoom, choosing exactly which part of the photo becomes their profile picture,
 * rather than the app silently squashing or auto-cropping it for them.
 */
class CropImageView(context: Context) : View(context) {
    ...
    private fun setupInitialTransform() {
        ...
        // "Cover" scale: the smallest scale at which the bitmap still fully fills the frame.
    ...
    /** Keeps the frame fully covered: the bitmap's edges may never move inside the frame's edges. */
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Dim everything outside the frame using four plain rectangles (avoids relying on any
        // canvas-clipping API, so this works identically on every supported Android version).
    ...
    /** Renders exactly what's currently inside the crop frame into a new square bitmap. */
