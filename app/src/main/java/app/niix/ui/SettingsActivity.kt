...
class SettingsActivity : SecureActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.text_app_version).text =
            getString(R.string.setting_app_version, BuildConfig.VERSION_NAME, BuildConfig.VERSION_CODE)
    }
    ...
    private fun checkForUpdateNow() {
        ...
        // Tor's SOCKS listener is the only route out. If it isn't up yet -- the app was just
        // opened, or Tor is still bootstrapping -- there's nothing to connect through, and
        // saying so plainly is far better than letting the attempt fail with a generic
        // "couldn't reach the update server", which reads like the server is down rather than
        // "not connected yet, try in a moment".
        val socks = container.transport.socksAddress()
        if (socks == null) {
            toast(getString(R.string.update_not_connected))
            return
        }
        val checker = app.niix.update.UpdateChecker(applicationContext, socksHost = socks.first, socksPort = socks.second)
