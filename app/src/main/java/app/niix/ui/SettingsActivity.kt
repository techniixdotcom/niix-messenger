...
class SettingsActivity : SecureActivity() {
 ...
 // Cancel button with no way to do anything. The explanation is the last item instead.
    ...
    /**
     * Erases the device of a contact who nominated this one.
     *
     * Only contacts who have nominated this device appear. Listing everyone and asking for a code
     * put the user in the position of guessing who had authorised them; the nominee's own device
     * already told us, so the list is the answer rather than a prompt.
     */
    ...
    /**
     * Final confirmation before erasing someone else's device.
     *
     * On purpose a separate step naming the person. This destroys their data with no undo and
     * no acknowledgement coming back, so it should not be reachable by one careless tap.
     */
        ...
        lifecycleScope.launch {
            ...
            AlertDialog.Builder(this@SettingsActivity)
                ...
                .setItems(labels.toTypedArray()) { _, which ->
                    ...
                    container.storage.remoteWipe.nominate(onion)
                    refreshRemoteWipeStatus()
