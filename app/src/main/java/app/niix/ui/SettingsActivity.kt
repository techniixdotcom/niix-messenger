...
class SettingsActivity : SecureActivity() {
 ...
 // and turn both on together, which is what they were trying to achieve.
        ...
        val batterySwitch = findViewById<MaterialSwitch>(R.id.switch_battery_log)
        batterySwitch.isChecked = container.storage.batteryLog.isEnabled()
        batterySwitch.setOnCheckedChangeListener { _, checked ->
            container.storage.batteryLog.setEnabled(checked)
        }

        findViewById<LinearLayout>(R.id.row_battery_report).setOnClickListener { showBatteryReport() }
 ...
 // does nothing when tapped is worse than one that admits it.
        ...
        val relayKeepsSwitch = findViewById<MaterialSwitch>(R.id.switch_relay_keeps_connection)
        relayKeepsSwitch.isChecked =
            container.storage.settings.getBool(SettingsStore.KEY_RELAY_KEEPS_CONNECTION, true)
        relayKeepsSwitch.setOnCheckedChangeListener { _, checked ->
            container.storage.settings.setBool(SettingsStore.KEY_RELAY_KEEPS_CONNECTION, checked)
        }

        val sleepSwitch = findViewById<MaterialSwitch>(R.id.switch_sleep_idle)
        sleepSwitch.isChecked = container.storage.settings.getBool(SettingsStore.KEY_SLEEP_WHEN_IDLE, true)
        sleepSwitch.setOnCheckedChangeListener { _, checked ->
            container.storage.settings.setBool(SettingsStore.KEY_SLEEP_WHEN_IDLE, checked)
            // Turning it off means staying connected, so cancel any pending wakeup. Turning it on
            // takes effect the next time the screen goes off.
            if (!checked) app.niix.SleepPoller.cancel(this)
        }
        ...
        findViewById<LinearLayout>(R.id.row_source).setOnClickListener {
            // Opens in a browser, not in-app. An embedded webview inside a messenger whose whole
            // point is not talking to servers would be a poor look and a worse habit to teach.
            runCatching {
                startActivity(
                    android.content.Intent(
                        android.content.Intent.ACTION_VIEW,
                        android.net.Uri.parse(getString(R.string.setting_source_full_url)),
                    ),
                )
            }.onFailure { toast(getString(R.string.setting_source_no_browser)) }
        }
        ...
        findViewById<LinearLayout>(R.id.row_donate).setOnClickListener {
            val address = getString(R.string.donate_monero_address)
            AlertDialog.Builder(this)
                .setTitle(R.string.setting_donate)
                .setMessage(getString(R.string.donate_monero_body, address))
                .setPositiveButton(R.string.donate_copy) { _, _ ->
                    // Copying rather than opening a wallet URI. A monero: link hands the address
                    // to whichever app claims the scheme, and an address that reaches the wrong
                    // app is an address someone else can substitute. The user pasting it into
                    // their own wallet keeps that choice with them.
                    val clip = android.content.ClipData.newPlainText("Monero address", address)
                    getSystemService(android.content.ClipboardManager::class.java)?.setPrimaryClip(clip)
                    toast(getString(R.string.donate_copied))
                }
 ...
 // below has something to return from.
    ...
    /** Both back controls go through here so they cannot diverge. From a section it returns to
     * the category list; from the list it leaves settings. */
    ...
    private fun showBatteryReport() {
        val report = runCatching { container.storage.batteryLog.render() }
            .getOrDefault(getString(R.string.battery_report_empty))
        val body = android.widget.TextView(this).apply {
            text = report
            // Monospace and small: the report is columns of durations, and a proportional font
            // makes them impossible to scan.
        ...
        AlertDialog.Builder(this)
            .setTitle(R.string.setting_battery_report)
            .setView(android.widget.ScrollView(this).apply { addView(body) })
            .setPositiveButton(R.string.diagnostics_copy) { _, _ ->
                getSystemService(android.content.ClipboardManager::class.java)
                    ?.setPrimaryClip(android.content.ClipData.newPlainText("battery report", report))
                toast(getString(R.string.battery_report_copied))
            }
            .setNegativeButton(R.string.dialog_ok, null)
            .show()
    }
 ...
 // On purpose a plain label, not the content: on Android 13+ the system shows a
 ...
 // Cancel button with no way to do anything. The explanation is the last item instead.
    ...
    private fun sendWipeChooseContact() {
        lifecycleScope.launch {
            val allowed = withContext(Dispatchers.IO) { container.conversations.contactsThisDeviceCanWipe() }
            if (allowed.isEmpty()) {
                AlertDialog.Builder(this@SettingsActivity)
                    .setTitle(R.string.remote_wipe_send)
                    .setMessage(R.string.remote_wipe_nobody_nominated_you)
                    .setPositiveButton(R.string.dialog_ok, null)
                    .show()
                return@launch
            }
            val contacts = withContext(Dispatchers.IO) { container.conversations.listContacts() }
            val labels = allowed.map { onion ->
                contacts.firstOrNull { it.onionAddress.value == onion }?.displayName
                    ?: (onion.take(12) + "\u2026")
            }
            AlertDialog.Builder(this@SettingsActivity)
                .setTitle(R.string.remote_wipe_send)
                .setItems(labels.toTypedArray()) { _, which ->
                    confirmSendWipe(labels[which], allowed[which])
                }
                .setNegativeButton(R.string.dialog_cancel, null)
                .show()
        }
    ...
    private fun confirmSendWipe(displayName: String, onion: String) {
        AlertDialog.Builder(this)
            .setTitle(R.string.remote_wipe_send)
            .setMessage(getString(R.string.remote_wipe_send_body, displayName))
            .setPositiveButton(R.string.remote_wipe_send_confirm) { _, _ ->
                lifecycleScope.launch {
                    container.conversations.sendRemoteWipe(onion)
 // "sent", not "wiped": nothing comes back by design, so anything stronger
    ...
    private fun nominateDialog() {
        lifecycleScope.launch {
            ...
            AlertDialog.Builder(this@SettingsActivity)
                ...
                .setItems(labels.toTypedArray()) { _, which ->
                    val onion = contacts[which].onionAddress.value
                    val idKey = container.conversations.identityKeyFor(onion)
                    if (idKey == null) {
                        // No session yet means no identity to bind the permission to, and an
                        // unbound permission is the thing H-07 was about.
                        toast(getString(R.string.remote_wipe_no_identity))
                        return@setItems
                    }
                    container.storage.remoteWipe.nominate(onion, idKey)
                    refreshRemoteWipeStatus()
 // Told immediately: until they know, they cannot act on it, and the
 // nomination exists only in this device's head.
                    lifecycleScope.launch { container.conversations.sendWipeNomination(onion, true) }
    ...
    private fun manageNominationsDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(labels.toTypedArray()) { _, which ->
                val onion = nominated[which]
                container.storage.remoteWipe.revoke(onion)
                refreshRemoteWipeStatus()
 // Revocation has to reach them too, or their app keeps offering to wipe a
 // device that no longer accepts it.
                lifecycleScope.launch { container.conversations.sendWipeNomination(onion, false) }
