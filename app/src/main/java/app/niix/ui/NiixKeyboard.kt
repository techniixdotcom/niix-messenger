...
/**
 * A soft keyboard drawn entirely by this app: every key is a plain [Button] built and handled
 * here, so no system input method -- including whatever third-party keyboard the person has
 * installed -- is ever invoked while composing a message. There is nothing for a keyboard app to
 * see or learn from, because none is ever asked to run.
 *
 * Deliberately simple: lowercase/uppercase letters (shift) and a numbers+punctuation page
 * (covering the symbols people actually use day to day in chat), not a full multi-page symbol
 * keyboard.
 */
class NiixKeyboard(
