...
class HomeActivity : SecureActivity() {
    ...
    private fun maybeAutoCheckForUpdate() {
        lifecycleScope.launch {
            ...
            // Silently skip if Tor isn't up yet -- this is a background check the user didn't
            // ask for, so there's nothing worth interrupting them about. Deliberately does NOT
            // record the check timestamp in that case, so the next attempt happens as soon as
            // the app is opened with a working connection rather than being suppressed for the
            // full interval by an attempt that never actually reached the network.
            val socks = container.transport.socksAddress() ?: return@launch
