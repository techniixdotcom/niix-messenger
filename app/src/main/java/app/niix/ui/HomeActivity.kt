...
class HomeActivity : SecureActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        PullToRefresh.attach(this, findViewById(R.id.home_refresh), container) { load() }
        ...
        findViewById<android.widget.ImageView>(R.id.action_wipe).setOnClickListener { wipeNow() }
    ...
    private fun wipeNow() {
        if (container.wipeWithoutConfirmation()) {
            performWipe()
            return
        }
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton(R.string.wipe_now_confirm) { _, _ -> performWipe() }
    ...
    private fun performWipe() {
        lifecycleScope.launch {
            withContext(Dispatchers.IO) {
                ...
                app.niix.CalculatorMemory.clear(this@HomeActivity)
            }
            container.applyLauncherIcon()
 // Straight to the lock screen with the task cleared, so nothing behind this activity
 // is still holding data that has just been destroyed.
            startActivity(
                Intent(
                    this@HomeActivity,
    ...
    private fun changeNameDialog() {
        lifecycleScope.launch {
            val current = withContext(Dispatchers.IO) { container.conversations.username() }
            val field = android.widget.EditText(this@HomeActivity).apply {
                setText(current)
                setSelection(text.length)
                inputType = android.text.InputType.TYPE_CLASS_TEXT
            }
            val padded = android.widget.LinearLayout(this@HomeActivity).apply {
            ...
            androidx.appcompat.app.AlertDialog.Builder(this@HomeActivity)
                ...
                .setMessage(R.string.profile_change_name_body)
                .setView(padded)
                .setPositiveButton(R.string.dialog_ok) { _, _ ->
                    lifecycleScope.launch {
                        val ok = container.conversations.setUsername(field.text.toString())
                        Toast.makeText(
                            this@HomeActivity,
                            getString(if (ok) R.string.profile_name_changed else R.string.profile_name_invalid),
                            Toast.LENGTH_SHORT,
                        ).show()
                        load()
                    }
    ...
    private fun showProfileMenu() {
        val options = arrayOf(
            getString(R.string.profile_change_name),
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setItems(options) { _, which ->
                when (which) {
                    ...
                    3 -> startActivity(Intent(this, MyCodeActivity::class.java))
                }
            }
            .show()
    }
    ...
    private fun maybeAutoCheckForUpdate() {
        lifecycleScope.launch {
            ...
            // Checks on every start. There used to be a once-per-interval gate, which meant a
            // security fix could sit unoffered for hours on a phone that had been opened and
            // closed repeatedly. The request is one small HTTPS GET, so the cost of doing it
            // every time is not worth the delay it buys.
            ...
            // A Tor-routed check needs Tor up. Skip quietly if it is not, rather than nagging
            ...
            // Still recorded, so Settings can show when the last check happened. It no longer
            // gates anything.
            withContext(Dispatchers.IO) {
                container.storage.settings.setLong(
                    app.niix.core.storage.SettingsStore.KEY_LAST_UPDATE_CHECK_AT,
    ...
    private fun downloadAutoUpdate(checker: app.niix.update.UpdateChecker, info: app.niix.update.UpdateInfo) {
        UpdateDownloadDialog.start(this, checker, info) { apkFile ->
            val startError = checker.promptInstall(apkFile)
            if (startError != null) {
                Toast.makeText(this, startError, Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    override fun onResume() {
        ...
        // Outside the initialised guard: with idle sleeping on, the service is stopped while the
        // screen is off, so coming back to the app has to bring it up again. Starting an already
        // running service is a no-op, so doing this every resume costs nothing.
    ...
    private fun load() {
        lifecycleScope.launch {
            // Read on the IO thread with the rest, and defensively.
            //
            // This was reading the encrypted database on the main thread, outside the IO block.
            // If the database was not open yet the read threw, load() died with it, and the
            // screen stayed empty forever with nothing logged. Defaulting to blurred is the safe
            // answer when the setting cannot be read: it hides content rather than exposing it.
            val blurEnabled = withContext(Dispatchers.IO) {
                runCatching {
                    container.storage.settings.getBool(SettingsStore.KEY_BLUR_MESSAGES, true)
                }.getOrDefault(true)
            }
    ...
    private fun requestBatteryExemptionOnce() {
        // Runs straight from onResume, so a throw here takes the whole screen down with it. The
        // database may not be open yet; a prompt that does not appear is a far smaller problem
        // than an app that never draws.
        val alreadyAsked = runCatching {
            container.storage.settings.getBool(app.niix.core.storage.SettingsStore.KEY_BATTERY_ASKED, false)
        }.getOrDefault(true)
