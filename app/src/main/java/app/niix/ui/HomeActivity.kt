...
class HomeActivity : SecureActivity() {
    ...
    /**
     * Erases everything, from the main screen.
     *
     * The confirmation is on by default and skippable only by an explicit setting. The reason
     * this button exists is to be reachable in a hurry, and a control that is easy to reach in a
     * hurry is equally easy to hit by accident, so the default has to be the safe one, and
     * removing the guard has to be a decision someone made in advance rather than in the moment.
     */
    private fun wipeNow() {
 ...
 // is still holding data that has just been destroyed.
    ...
    /**
     * Profile actions, from the avatar in the header.
     *
     * Photo actions hand off to SettingsActivity instead of duplicating the picker here, which
     * needs a registered ActivityResultLauncher to work at all.
     */
