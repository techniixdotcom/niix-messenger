...
/**
 * The decoy calculator.
 *
 * Deliberately does NOT apply screenshot protection, unlike every other screen. A calculator
 * that produces a black rectangle in screenshots and can't be screen-recorded is conspicuous --
 * it would signal that this isn't really a calculator, which defeats the disguise it exists to
 * provide. Nothing sensitive is on screen here anyway: a screenshot shows a calculator.
 */
    ...
    private fun tryUnlock(raw: String) {
        lifecycleScope.launch {
            ...
            if (opened) {
                expr.clear()
                render()
                startActivity(Intent(this@CalculatorActivity, SplashActivity::class.java))
                finish()
            }
        }
    }
