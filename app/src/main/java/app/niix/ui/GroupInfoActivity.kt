...
class GroupInfoActivity : SecureActivity() {
    ...
    private fun memberRow(onion: String, name: String, role: GroupRole): TextView {
        ...
        return TextView(this).apply {
            ...
            // Tapping opens member actions for an admin, and the safety number for everyone
            // else. Verification used to be unreachable in a group at all: the toolbar item
            // computes a number for the conversation, and a group's id is not a contact, so it
            // always failed with "no session". Safety numbers are pairwise, so a group can only
            // have one per member.
            if (amAdmin) {
                setOnClickListener { showMemberActions(onion, name, role) }
            } else if (onion != container.selfOnion) {
                setOnClickListener { showMemberSafetyNumber(onion, name) }
            }
    ...
    /**
     * The safety number for one member.
     *
     * Needs a session with that member, which exists only once messages have been exchanged
     * directly. Being in the same group is not enough: group messages use sender keys and never
     * establish the pairwise session a fingerprint is computed from. The message says that
     * rather than implying something is broken.
     */
    private fun showMemberSafetyNumber(onion: String, name: String) {
        lifecycleScope.launch {
            val number = withContext(Dispatchers.IO) {
                runCatching { container.conversations.safetyNumber(onion) }.getOrNull()
            }
            if (number == null) {
                androidx.appcompat.app.AlertDialog.Builder(this@GroupInfoActivity)
                    .setTitle(name)
                    .setMessage(R.string.group_verify_needs_session)
                    .setPositiveButton(R.string.dialog_ok, null)
                    .show()
                return@launch
            }
            val body = TextView(this@GroupInfoActivity).apply {
                text = app.niix.core.crypto.SafetyNumber.formatted(number)
            ...
            androidx.appcompat.app.AlertDialog.Builder(this@GroupInfoActivity)
                .setTitle(getString(R.string.group_verify_title, name))
                .setView(body)
                .setMessage(R.string.group_verify_body)
                .setPositiveButton(R.string.dialog_ok, null)
                .show()
        }
    }
