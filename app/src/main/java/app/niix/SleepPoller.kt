...
object SleepPoller {
    ...
    fun schedule(context: Context) {
        runCatching {
            (context.applicationContext as NiixApp).container.storage.batteryLog
                .record("alarm scheduled for ${intervalMillis(context) / 60_000} minutes")
        }
    ...
    class Receiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent?) {
            ...
            // Recorded so the gap between wakeups can be compared against the interval that was
            // asked for. A phone that silently drops alarms shows up here as intervals far longer
            // than scheduled, which is the difference between the design being wrong and the
            // vendor ignoring it.
            runCatching {
                (context.applicationContext as NiixApp).container.storage.batteryLog
                    .record("alarm fired")
