...
object TempFileGuard {
    ...
    // "att_" is a prefix of "att_crop_", so both are covered -- but they are listed separately
    // because relying on that overlap is how att_crop_ was missed in the first place when it
    // was written to a different directory.
    // "open_" already prefixes "open_avatar_", but both are listed because relying on that
    // overlap is exactly how att_crop_ came to be missed when it moved directories.
    private val PREFIXES = listOf("cam_", "rec_", "att_", "att_crop_", "open_", "open_avatar_")
