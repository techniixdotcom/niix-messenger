...
class ConnectivityService : Service() {
    ...
    /** Whether the onion service is currently published and reachable. */
    ...
    /**
     * Stops the service once a polling wakeup has connected and had time to drain.
     *
     * Waits for the connection rather than for a fixed period. This used to be a flat two
     * minutes, which measurement showed was the wrong shape: bootstrap is usually 3 or 4 seconds
     * but was once 83, so a fixed window spent most of its time waiting on a fast connection and
     * risked stopping before a slow one had finished. A wakeup that disconnects before fetching
     * anything is worse than no wakeup, because the next chance is another full interval away.
     *
     * The cap still applies, so a connection that never succeeds does not hold the radio open
     * indefinitely.
     */
    private fun schedulePollShutdown(container: AppContainer) {
        ...
        pollShutdownJob = CoroutineScope(Dispatchers.Default).launch {
            val deadline = android.os.SystemClock.elapsedRealtime() + POLL_WINDOW_MILLIS
            var connected = false
            while (android.os.SystemClock.elapsedRealtime() < deadline) {
                // Set once the onion service has actually published, which is the point the
                // connection is usable. Bootstrap reaching 100 is earlier than that and would
                // start the drain before anything could arrive. Kept separate from
                // container.selfOnion, which is the user's own address and has to stay readable
                // while offline -- clearing that would empty the QR code on the "my code" screen.
                if (networkReady) { connected = true; break }
                delay(1_000)
            }

            if (connected) {
                // Time to actually fetch once the connection exists. Measured from connecting,
                // not from waking, so a slow bootstrap does not eat the fetch.
                delay(POLL_DRAIN_MILLIS)
                battery(container, "poll complete: connected and drained")
            } else {
                battery(container, "poll gave up: never connected within the window")
            }
    ...
    private fun whyNotSleeping(container: AppContainer): String = runCatching {
        when {
            container.relay.isHostingEnabled() -> "relay hosting is on and set to stay connected"
            !container.storage.settings.getBool(
    ...
    private fun shouldSleep(container: AppContainer): Boolean = runCatching {
        // Hosting a relay blocks sleeping only if the user asked it to.
        //
        // This used to be unconditional, on the reasoning that an offline relay quietly fails the
        // peers depending on it. That is true, but it made the choice for them: measurement
        // showed every single screen-off on a relay-hosting device refusing to sleep, so the
        // battery setting did nothing at all and nothing said why.
        //
        // A part-time relay is still useful. Senders fall back to other relays when one is
        // unreachable, and envelopes already stored here survive the sleep and are served on the
        // next wake. Defaults to keeping the connection, so nobody's relay silently becomes
        // part-time because of an update.
        if (container.relay.isHostingEnabled() &&
            container.storage.settings.getBool(
                app.niix.core.storage.SettingsStore.KEY_RELAY_KEEPS_CONNECTION,
                true,
            )
        ) {
            return false
 ...
 // is only accepted there, and DATA_SYNC is what 29 to 33 expect.
    ...
    companion object {
        ...
        /** Longest a polling wakeup waits for a connection before giving up. Bootstrap was
         * measured at 3-83 seconds, so this has to tolerate the slow end. */
        private const val POLL_WINDOW_MILLIS = 3L * 60 * 1000

        /** Time allowed to collect messages once connected, measured from connecting. */
        private const val POLL_DRAIN_MILLIS = 30L * 1000
