...
class UpdateChecker(
    ...
    suspend fun checkForUpdate(currentVersionName: String): UpdateCheckResult {
        ...
        if (tagName.isBlank()) return UpdateCheckResult.UpToDate

 // Require the offered version to be strictly greater, not merely different.
 //
 // Comparing for inequality alone accepts an older release as an update. Every published
 // release is correctly signed, so an attacker who can influence which one the client
 // sees, a stale mirror, a manipulated response, could roll the app back to a version
 // with a known flaw without breaking any signature. The signature proves authorship, not
 // freshness; only the version comparison provides that.
        if (!isStrictlyNewerForTest(tagName, currentVersionName)) return UpdateCheckResult.UpToDate
    ...
    /**
     * Downloads, verifies and stages an update.
     *
     * [onProgress] receives bytes received and the total, or -1 for the total when the server
     * does not declare a length. [onVerifying] is called once the download is complete and the
     * signature check begins. [isCancelled] is polled between chunks; returning true stops the
     * download, deletes the partial file and returns [UpdateInstallResult.Rejected].
     */
 ...
 // partial or unverified file can never be sitting at the path the installer uses.
        ...
        if (!downloadToFile(info.apkUrl, partFile, onProgress = onProgress, isCancelled = isCancelled)) {
            runCatching { partFile.delete() }
            if (isCancelled()) {
                DiagnosticLog.record("update", "download cancelled by the user")
                return UpdateInstallResult.Rejected("Download cancelled")
            }
            ...
            try {
                ...
                connection.inputStream.use { input ->
                    dest.outputStream().use { output ->
                        ...
                        while (true) {
                            // Checked per chunk, so Cancel takes effect within one 64 KB read
 ...
 // Clearnet is the default here, chosen : an update that won't download is an
 ...
 // anything is actually wrong. Nothing to log: this app has no
 ...
 // is silently retried rather than recorded.
    ...
    companion object {
        ...
        /**
         * True if [offered] is strictly later than [current].
         *
         * Numeric per segment, not string compare: "0.8.10" sorts before "0.8.9" as text but is
         * the later release. Unparseable means not newer. Refusing an update is recoverable;
         * taking a downgrade is not.
         *
         * In the companion so it is testable without a network stack.
         */
        internal fun isStrictlyNewerForTest(offered: String, current: String): Boolean {
            fun parse(v: String): List<Int>? {
                val cleaned = v.trim().removePrefix("v")
                if (cleaned.isEmpty()) return null
                val parts = cleaned.split('.')
                val nums = parts.map { part -> part.toIntOrNull()?.takeIf { it >= 0 } ?: return null }
                return nums.takeIf { it.isNotEmpty() }
            }
            val a = parse(offered) ?: return false
