...
class UpdateChecker(
 ...
 // is silently retried rather than recorded.
    ...
    companion object {
        ...
        /**
         * Whether [offered] is a strictly later version than [current].
         *
         * Compares numerically, segment by segment, because version names are not orderable as
         * text: "0.8.10" sorts before "0.8.9" as a string while being the later release.
         * Anything that cannot be parsed as numeric segments is treated as not newer, refusing
         * an update is recoverable, accepting a downgrade is not.
         *
         * In the companion rather than the instance so it can be tested without constructing an
         * UpdateChecker, which would otherwise need a network stack to exist.
         */
         ...
         * On purpose far smaller than [MAX_RESPONSE_BYTES]: nothing on that path is ever
