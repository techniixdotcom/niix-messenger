...
/**
 * Tests that only mean anything on a device.
 *
 * The JVM suite is large and green, and says nothing about whether the encryption paths work on
 * real hardware, the Android Keystore, SQLCipher's native library and actual file I/O are all
 * absent there. These fill that gap.
 *
 * On purpose limited to APIs the app module can genuinely reach. Several storage internals are
 * `internal` to core:storage, and widening their visibility so a test could reach them would mean
 * loosening production encapsulation to suit the tests, those belong in core:storage's own
 * androidTest source set instead.
 *
 * Run with:  ./build-niix.sh --device-tests
 */
