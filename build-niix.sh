...
# Bumps version.properties in place: versionCode by 1, versionName's patch segment by 1
# (X.Y.Z -> X.Y.(Z+1); anything that isn't three dot-separated numbers is left as-is and only
# the versionCode is bumped). Exports NIIX_VERSION_NAME/NIIX_VERSION_CODE so the build that
# follows actually uses the new values -- this is what makes every published build strictly
# newer than the last, which Android's own installer requires for an "update" to be accepted
# at all (this exact gap -- versionCode silently defaulting to 1 forever -- is why "app not
# installed as package appears to be invalid" happened: every build looked identical to Android).
bump_version() {
  local old_name old_code new_name new_code
...
publish_release() {
  ...
  echo "==> Removing previous releases from $repo_slug before publishing $tag..."
  local existing_tag
  while IFS= read -r existing_tag; do
    ...
    echo "    Deleting release $existing_tag..."
    gh release delete "$existing_tag" --repo "$repo_slug" --yes --cleanup-tag || true
  done < <(gh release list --repo "$repo_slug" --json tagName --jq '.[].tagName' 2>/dev/null)
  ...
  echo "    (Source replaced, then APK + .sig + SHA256SUMS.txt uploaded together.)"
}
