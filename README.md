...
`--publish` asks for a version name and code before building (suggesting the next patch
version and next code as defaults, so you can usually just press enter) — `version.properties`
at the project root is the single source of truth the build actually uses, and a version code
that isn't strictly higher than the current one is refused outright, since Android won't accept
it as an update either way. The built APK is renamed to `niix-messenger-<version>.apk`.
...
through `gh auth setup-git` rather than a separately-managed token. The new release is then
created with the signed APK, its `.sig`, and `SHA256SUMS.txt` — existing releases are left
alone; only the source gets replaced, never release history. Before any of this, a staged copy
of the tree is built with every secret and build artifact stripped out and verified absent --
`keystore.properties`, `*.jks`, `*.pem`, `local.properties`, `build.log`, `.git`, `.gradle`,
`.toolchain`, and every `build/` directory are never included, checked after the fact rather
than just assumed, and the whole run stops before pushing or publishing anything if that check
ever fails.

The script downloads the JDK, Gradle, and Android SDK on its own if they aren't already on your
machine — no Android Studio or manual setup required. These go into `~/.niix-toolchain` (set
`NIIX_TOOLCHAIN_DIR` to put them elsewhere), not into the project folder, so deleting or
re-extracting the project doesn't mean downloading several hundred megabytes again. Nothing is
installed system-wide and nothing needs sudo. Every run writes a full copy of everything
printed to `build.log` in the project root, and every built APK is verified with `apksigner`
(the same structural/signature check Android's own installer runs) immediately after building —
a broken APK is caught right there with a clear reason, not just discovered later as a vague
"problem parsing the package" on someone's phone.

`build-niix.sh` (this file, with the publish/`gh release` tooling) is this maintainer's own —
what gets pushed to the public repo when publishing is `build-niix-public.sh`, swapped in as
`build-niix.sh`, with everything about publishing, deleting releases, or pushing source removed
entirely. Cloning the repo gets you a real, complete, independently-testable build script with
nothing missing for actually building the app — just none of the release-management tooling.
